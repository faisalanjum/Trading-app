...
class Alpaca:
    ...
    def getData(self,request_for,symbol,start,end=str(datetime.now().strftime('%Y-%m-%d')),limit=10000,timeframe="1Min",save_to="filesystem"):
        errors_to_handle=[aiohttp.client_exceptions.ClientOSError,aiohttp.client_exceptions.ServerDisconnectedError, asyncio.exceptions.TimeoutError,aiohttp.client_exceptions.ClientConnectorError]
    
        try:
           data= asyncio.run(self.gatherData(request_for,symbol,start,end,limit,timeframe,save_to))
           return data
        except Exception as e:
            if e.__class__ in errors_to_handle:
